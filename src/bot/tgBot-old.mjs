...
export class TgBotOld {
  ...
  async handleArticleInput(context, productId) {
    ...
    const subscribedOptionIds = subscription?.optionIds || []

    const sizeButtons = product.sizes.map(size => {
      const isSubscribed = subscribedOptionIds.includes(size.optionId)
      const label = isSubscribed
        ? `❌ ${size.name} (${size.currentPrice}₽)`
        : `✅ ${size.name} (${size.currentPrice}₽)`

      const action = isSubscribed
        ? `unsub:${productId}:${size.optionId}`
        : `sub:${productId}:${size.optionId}`
    ...
    const msg = await context.replyWithPhoto(cardMessage.photo, {
      ...
      reply_markup: { inline_keyboard: sizeButtons }
    })
  ...
  async handleSubscribe(context) {
    const userId = context.from.id
    const [, productIdRaw, optionIdRaw] = context.match[0].split(':')
    ...
    const existing = subscriptions.find(s => s.productId === productId)
    ...
    if (existing) {
      ...
      subscriptions.push({ productId, optionIds: [optionId] })
    }
    ...
    await context.answerCbQuery(`Подписался на размер (optionId: ${optionId}) 👀`)
    console.log(`[TgBot] user ${userId} subscribed to ${productId}:${optionId}`)
  }
  ...
  formatProductCard(product) {
    ...
    const sizesInfo = (product.sizes || [])
      .map(size => {
        const price = size.currentPrice
          ? `${size.currentPrice.toLocaleString()} ₽`
          : '—'
  ...
  async handleUnsubExecute(context) {
    ...
    const existing = subscriptions.find(s => s.productId === productId)
    if (existing) {
      existing.optionIds = existing.optionIds.filter(id => id !== optionId)

      if (existing.optionIds.length === 0) {
        // Удаляем товар, если больше нет подписанных размеров
        const index = subscriptions.findIndex(s => s.productId === productId)
    ...
    await context.reply(`Ты отписался от ${productId}, размер optionId: ${optionId} ❌`) // TODO

    console.log(`[TgBot] user ${userId} unsubscribed from ${productId}:${optionId}`)
  }
  ...
  async notifyPriceChange(user, product, changes) {
    ...
    const diffLines = changes.map(c => {
      const arrow = c.currentPrice < c.prevPrice ? '⬇️' : '⬆️'

      return `${arrow} ${c.name}: ${c.prevPrice.toLocaleString()} ₽ → ${c.currentPrice.toLocaleString()} ₽`
    })
    ...
    try {
      ...
      const msg = await this.bot.telegram.sendPhoto(chatId, cardMessage.photo, {
        ...
        reply_markup: {
          inline_keyboard: [
            [{ text: '❌ Отписаться', callback_data: `unsub_all:${product.id}` }]
          ]
        }
      })
